...
void Editor::BlueprintEditor::Deinit()
{
    Instance::Deinit();
    ecs.Deinit();
    config.SaveConfig();
...
void Editor::BlueprintEditor::SetPendingBP(const ResBlueprint &InBP)
{
    pendingBP = InBP;
}

void Editor::BlueprintEditor::SetBP(const ResBlueprint& InBP)
{
...
void Editor::BlueprintEditor::Frame()
{
    ecs.Frame();
...
void Editor::BlueprintEditor::DrawPanel()
{
    if (instanceID == ECS::INVALID_ID)
        return; 
