...
void Editor::AssetBrowser::TryStartThread()
{
    if (!running)
    {
        running = true;
...
void Editor::AssetBrowser::TryStopThread()
{
    if (running)
    {
        running = false;
...
void Editor::AssetBrowser::ForceUpdateCache()
{
    PROFILE();
    if (!editLocked)
        rootLock.lock();
...
void Editor::AssetBrowser::SelectAsset(const String &InPath)
{
...
Set<String> Editor::AssetBrowser::GetSelected()
{
    return config.Selected.Get();
}

String Editor::AssetBrowser::GetNodeType(const Node& InNode)
{
    if (InNode.ext == ".json")
    {
...
std::function<bool(const Editor::AssetBrowser::Node&, const Editor::AssetBrowser::Node&)> Editor::AssetBrowser::GetSortFunc() const
{
    switch (sort)
    {
...
void Editor::AssetBrowser::SelectNode(const Node& InNode)
{
    PROFILE();
...
void Editor::AssetBrowser::NodeDragDrop(const Node &InNode)
{
    // Drag drop
...
void Editor::AssetBrowser::NodeContext(const Node &InNode)
{
    PROFILE();
...
void Editor::AssetBrowser::KeyboardShortcuts()
{
...
void Editor::AssetBrowser::BeginCut()
{
    LOG("Begin cut");
    String cutCmd = "CUT:";
    for (auto& s : config.Selected.Get())
...
void Editor::AssetBrowser::BeginCopy()
{
    LOG("Begin copy");
    String copyCmd = "CPY:";
    for (auto& s : config.Selected.Get())
...
void Editor::AssetBrowser::Paste()
{
...
void Editor::AssetBrowser::Duplicate()
{
    for (auto& s : config.Selected.Get())
...
void Editor::AssetBrowser::Delete()
{
    for (auto& s : config.Selected.Get())
...
void Editor::AssetBrowser::DrawNode(Node& InNode)
{
    PROFILE();
...
void Editor::AssetBrowser::DrawAddMenu()
{
    if (openAdd)
    {
